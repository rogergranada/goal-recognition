...
from os.path import exists, join, splitext, dirname, basename, realpath, isfile
...
class DecompressedFile(FileHandler):
    ...
    def first_occurrence(self, element):
        self.__enter__()
        for self.nb_line, line in enumerate(self.fin):
            if not line or not line[0].isdigit(): continue
            arr = self.check_line(self.nb_lines, line)
            idf, sub, rel, obj = arr[0], arr[1], arr[2], arr[3]
            if sub == element or obj == element:
                return idf
        return -1

    def relations_at_frame(self, id_frame):
        triplets = []
        self.__enter__()
        for self.nb_line, line in enumerate(self.fin):
            if not line or not line[0].isdigit(): continue
    ...
    def relations_up_to_frame(self, id_frame):
        dic = {}
        triplets = []
        self.__enter__()
        for self.nb_line, line in enumerate(self.fin):
            ...
            dic[sub] = (rel, obj)
            if idf > id_frame:
                triplets = [(sub, dic[sub][0], dic[sub][1]) for sub in dic]
                break
        return triplets
...
class PDDLInit(object):
    def __init__(self, initfile='pddl.ini'):
        self.init_states = ''
        self.groups = {}
        self.relations = {}
    ...
    def _load_relations(self):
        for rel in self.config['RELATIONS']:
            relation = self.config['RELATIONS'][rel]
            self.relations[rel] = ast.literal_eval(relation)
# End of PDDLInit class
...
class PDDLFile(object):
            ...
            for line in fin:
                ...
                if record_predicates:
                    if line == ')':
                        ...
                        self.predicates.append(line.strip())

                if record_action:
                    if line == ')':
                        self.actions[action_name] = description
                        description = ''
                        action_name = ''
                        record_action = False
                    else:
                ...
                if '(domain' in line:
                    self.domain = line.split('(domain ')[1].replace(')','')
                elif '(:requirements' in line:
                    self.requirements = line.replace(')', '').split()[1:]
                elif '(:predicates' in line:
                    record_predicates = True
                elif '(:action' in line:
                    record_action = True
                    action_name = line.split('action ')[1]
                    description = ''
    ...
    def _add_index(self, key):
        ...
        has_index = True
        while True:
            if not key+'-'+str(index) in self.actions:
                return key+'-'+str(index)
            index += 1

    def _minor_precondition(self, action_name, description):
        line_prec1 = self.actions[action_name].split('\n')[1]
        nb_prec1 = len(line_prec1.split())
        line_prec2 = description.split('\n')[1]
        nb_prec2 = len(line_prec2.split())
        if nb_prec1 <= nb_prec2:
            return self.actions[action_name]
        return description
                    
    def add_action(self, action_name, description, minor=False):
        if action_name in self.actions and description != self.actions[action_name]:
            if minor:
                description = self._minor_precondition(action_name, description)
            else:
                action_name = self._add_index(action_name)
        self.actions[action_name] = description

    def save_file(self, fname):
        # build strings before saving
        # requirements
        str_requirements = ''
        for req in self.requirements:
            str_requirements += req+' '
        # predicates
        str_predicates = ''
        for pred in sorted(self.predicates):
            str_predicates += '  {}\n'.format(pred)
        str_predicates = str_predicates[:-1]
        # actions
        str_actions = ''
        for action in sorted(self.actions):
            str_actions += '(:action {}\n{})\n\n'.format(action, self.actions[action])

        # save file
        tp = Template(open('pddl.template').read())
        str_out = tp.safe_substitute(domain=self.domain,
                                     requirements=str_requirements[:-1],
                                     predicates=str_predicates,
                                     actions=str_actions
        )
